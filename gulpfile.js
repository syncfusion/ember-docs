...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/ember-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/ember-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '') {
                ...
                if (fs.existsSync('../ember-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/ember-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../ember-docs/` + changedFileName, `../../../gitlabRepo/ember-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/ember-docs/')) {
                            shelljs.cp('-rf', `../ember-docs/` + changedFileName, `../../../gitlabRepo/ember-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/ember-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/ember-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/ember-docs`);
